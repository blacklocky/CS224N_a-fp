...
def naiveSoftmaxLossAndGradient(
    ...
    s_softed = softmax(s)
    loss = -np.log(s_softed[outsideWordIdx])
    prob = s_softed.copy().reshape(-1,1)#(vocab,1) 预测每个词的概率
    ture_uo = outsideVectors[outsideWordIdx] #(dim,)
    gradCenterVec = -ture_uo + np.sum(outsideVectors*prob,axis=0)#真实值 - 每个词的预测和
    gradOutsideVecs = np.dot(prob,centerWordVec[:,np.newaxis].T)#（vocab,1）@(1,dim)
...
def negSamplingLossAndGradient(
    ...
    '''s1 = -np.log(sigmoid(outsideVectors@centerWordVec))
    gradCenterVec = outsideVectors.T@(-(1-sigmoid(outsideVectors@centerWordVec)))
    gradOutsideVecs = -(1 - sigmoid(outsideVectors@centerWordVec)).reshape(-1,1)@centerWordVec.reshape(-1,1).T
    s2,gc2,= 0,0
    for ws in negSampleWordIndices:
        s2 -= np.log(sigmoid(-outsideVectors[ws]@centerWordVec)) 
        gc2 += (1-sigmoid(-outsideVectors[ws]@centerWordVec))*outsideVectors[ws]
        
    loss = s1[outsideWordIdx] + s2
    gradCenterVec += gc2'''
    scores = np.dot(outsideVectors[indices],centerWordVec).reshape(-1,1)
    score_o = scores[0] # float
    score_neg = -scores[1:] # (K_neg,)
    prob_uoc = sigmoid(score_o)
    prob_unegc = sigmoid(score_neg).reshape(-1,1)
    loss = -np.log(sigmoid(score_o)) - np.sum(np.log(sigmoid(score_neg)))
    gradCenterVec = -outsideVectors[outsideWordIdx]*(1 - prob_uoc)+np.sum(outsideVectors[negSampleWordIndices]*(1-prob_unegc),axis=0)
                                                                    
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec*(1-prob_uoc)
    #gradOutsideVecs[negSampleWordIndices] += centerWordVec*(1-prob_unegc)
    for i in negSampleWordIndices:
        gradOutsideVecs[i] += centerWordVec*(1-prob_unegc[negSampleWordIndices.index(i)])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    '''vc = centerWordVectors[word2Ind[currentCenterWord]]
    for outsideword in outsideWords:
        u_ind = word2Ind[outsideword]
        loss_ ,grad_c,grad_o = word2vecLossAndGradient(vc,u_ind,outsideVectors,dataset)
        loss += loss_
        gradCenterVecs[word2Ind[currentCenterWord]] += grad_c.flatten()
        gradOutsideVectors += grad_o 
    '''
    for outsideword in outsideWords:
        loss_i,grad_c,grad_u = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],
        word2Ind[outsideword],outsideVectors,dataset)
        loss += loss_i
        gradOutsideVectors += grad_u
        gradCenterVecs[word2Ind[currentCenterWord]] += grad_c
