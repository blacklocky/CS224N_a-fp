...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
         ...
         #   return
        if transition == "S":
            self.stack.append(self.buffter[0])
            self.buffter.pop(0)
        ...
        if transition == "LA":
            ...
            self.dependencies.append(tuple(wj,wi))'''
        ...
        if transition == "RA":
            ...
            self.dependencies.append(tuple(wi,wj))'''
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Initialize partialparses as a list of PartialParses, one for each sentence in sentences
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    # Initialize unfinishedparses as a shallow copy of partialparses
    unfinished_parses = partial_parses[:]
    
    # while unfinishedparses is not empty do
    while unfinished_parses:
        ...
        # Perform a parse_step on each partial parse in the minibatch with its predicted transition
        for partial_parse, transition in zip(mini_batch, transitions):
            ...
            if not partial_parse.buffter and len(partial_parse.stack) == 1:
...
def test_step(name, transition, stack, buf, deps,
    ...
    pp = PartialParse([])
    pp.stack, pp.buffter, pp.dependencies = stack, buf, deps

    pp.parse_step(transition)
    stack, buf, deps = (tuple(pp.stack), tuple(pp.buffter), tuple(sorted(pp.dependencies)))
...
class DummyModel(object):
    ...
    def unidirectional_predict(self, partial_parses):
        ...
        """
        return [("RA" if pp.stack[1] == "right" else "LA") if len(pp.buffter) == 0 else "S"
                for pp in partial_parses]

    def interleave_predict(self, partial_parses):
        """First shifts everything onto the stack and then interleaves "right" and "left".
        """
        return [("RA" if len(pp.stack) % 2 == 0 else "LA") if len(pp.buffter) == 0 else "S"
                for pp in partial_parses]
