...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        ### TODO:
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_normal_(torch.ones((self.embed_size*self.n_features,self.hidden_size))))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.zeros((self.hidden_size))))
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight =nn.Parameter(nn.init.xavier_normal_(torch.ones((self.hidden_size,self.n_classes)))) 
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.ones((self.n_classes))))
    ...
    def embedding_lookup(self, w):
        ...
        ### TODO:
        x = torch.index_select(self.embeddings,0,torch.flatten(w)).view(w.shape[0],w.shape[1]*self.embed_size)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.matmul(inputs,self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
